{s name=text/settings_main namespace=backend/bepado/view/main}
    <div class=bepado-home-page>
        <h3 class=headline>Einstellungen (Settings)</h3>
        <div class=content>
            <p>
                Konfigurieren Sie zunächst einige Einstellungen, die für importierende und exportierende Shops
                gleichermaßen von Bedeutung sind.
            </p>
            <ul>
                <li>
                    <strong>Basis Einstellungen</strong>
                    <p>
                        Hier verbinden Sie Ihren Shop mit  bepado und konfigurieren die Anzeige der Produkte in Ihrem
                        Warenkorb und auf der Artikeldetailseite für bepado.
                        <br />
                        API Schlüssel
                        Bitte hinterlegen Sie an dieser Stelle Ihren API Key, um eine Verbindung zu bepado aufzubauen.
                        Sie finden Ihren API-Key auf sn.bepado.de unterhalb der Einstellungen im Tab-Reiter Synchronisation.
                    </p>
                </li>

                <li>
                    <strong>Erweiterte Einstellungen</strong>
                    <p>
                        In diesem Bereich lassen sich weitere Einstellungen zum Bereich SEO, Debugging und besondere
                        Konfigurationen festlegen. Zu den SEO- Einstellungen zählt das Setzen eines noindex Tags für
                        bepado-Produkte, um Duplikate Content zu vermeiden.
                    </p>
                </li>
            </ul>
        </div>
    </div>
{/s}