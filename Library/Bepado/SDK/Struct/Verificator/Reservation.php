...
class Reservation extends Verificator
{
    ...
    public function verify(VerificatorDispatcher $dispatcher, Struct $struct)
    {
        ...
        foreach ($struct->messages as $shopId => $messages) {
            ...
            foreach ($messages as $message) {
                if (!$message instanceof Struct\Message) {
                    throw new \Bepado\SDK\Exception\VerificationFailedException('$message MUST be an instance of \\Bepado\\SDK\\Struct\\Message.');
                }
        ...
        if (!is_array($struct->orders)) {
            throw new \Bepado\SDK\Exception\VerificationFailedException('$orders MUST be an array.');
        }
        foreach ($struct->orders as $order) {
            if (!$order instanceof Struct\Order) {
                throw new \Bepado\SDK\Exception\VerificationFailedException('$orders MUST be an instance of \\Bepado\\SDK\\Struct\\Order.');
            }
